const htmlLib = require('./htmlLib');

const serveLoginPage = function (req, res) {
  let loginPage = this.fs.readFileSync('./templates/login.html', 'utf8');
  loginPage = loginPage.replace('MESSAGE', req.cookies.message || '');
  res.write(loginPage);
  res.end();
};

const redirectLoggedInUserToHome = function (req, res) {
  if (req.urlIsOneOf(['/', '/login']) && req.user) { res.redirect('/todolists'); }
};

const redirectLoggedOutUserToLogin = function (req, res) {
  const forbiddenUrls = ['/logout', '/todolists', '/todolist'];
  if (req.urlIsOneOf(forbiddenUrls) && !req.user) { res.redirect('/login'); }
};

const serveTodolists = function (req, res) {
  const filePath = './templates/todolists.html';
  let content = this.fs.readFileSync(filePath, 'utf8');
  const user = this.getUserData(req.user);
  const todoLists = user.getAllTodoLists();
  const todoListsHtml = htmlLib.convertAllTodoListsToHtml(todoLists);
  content = content.replace('TODOLISTS', todoListsHtml);
  res.setHeader('Content-Type', 'text/html');
  res.write(content);
  res.end();
};

const serveTodoItems = function (req, res) {
  const filePath = './templates/todoitems.html';
  let content = this.fs.readFileSync(filePath, 'utf8');
  const user = this.getUserData(req.user);
  const todoItems = user.getAllTodoItems(req.listId);
  let todoItemsHtml = '';
  if (todoItems) {
    todoItemsHtml = htmlLib.convertAllTodoItemsToHtml(todoItems);
  }
  content = content.replace('TODOITEMS', todoItemsHtml);
  res.setHeader('Content-Type', 'text/html');
  res.write(content);
  res.end();
};

const addTodoList = function (req, res) {
  const user = this.getUserData(req.user);
  const title = req.body.title;
  const description = req.body.description;
  user.addTodoList(title, description);
  res.redirect('/todolists');
};

const addTodoItem = function (req, res) {
  const item = req.body.objective;
  const listId = req.listId;
  const user = this.getUserData(req.user);
  user.addTodoItemTo(listId, item);
  res.redirect(`/todolist/${listId}`);
};

const setListId = function (req) {
  const url = req.url;
  const urlRegex = /^\/todolist\/[\d]+$/;
  if (urlRegex.test(url)) {
    const listId = +url.split('/').pop();
    req.listId = listId;
    req.url = '/todolist';
  }
};

const editTodoList = function (req, res) {
  const user = this.getUserData(req.user);
  const title = req.body.title;
  const listId = req.body.listId;
  user.updateTodoList(listId, title);
  res.setHeader('Content-Type', 'text/plain');
  res.end('success');
};

const deleteTodoList = function (req, res) {
  const user = this.getUserData(req.user);
  const listId = req.body.listId;
  user.deleteTodoList(listId);
  res.setHeader('Content-Type', 'text/plain');
  res.end('success');
};

const deleteTodoItem = function (req, res) {
  const user = this.getUserData(req.user);
  const listId = req.listId;
  const itemId = req.body.itemId;
  user.deleteTodoItem(listId, itemId);
  res.setHeader('Content-Type', 'text/plain');
  res.end('success');
};

const updateItemStatus = function (req, res, app) {
  const user = app.getUserData(req.user);
  const listId = req.listId;
  const itemId = req.body.itemId;
  user.changeStatus(listId, itemId);
  res.setHeader('Content-Type', 'text/plain');
  res.end('success');
};

const editItemObjective = function (req, res, app) {
  const user = app.getUserData(req.user);
  const listId = req.listId;
  const itemId = req.body.itemId;
  const objective = req.body.objective;
  user.editTodoItemObjective(listId, itemId, objective);
  res.setHeader('Content-Type', 'text/plain');
  res.end('success');
};
const editActions = {
  changeStatus: updateItemStatus,
  editItemObjective
};

const editTodoItem = function (req, res) {
  const action = req.body.action;
  if (editActions[action]) {
    editActions[action](req, res, this);
  }
  else { res.end('failed'); }
};

const loadUser = function(req){
  const sessionid = req.cookies.sessionid;
  const user = this.sessionManager.getUserBySessionId(sessionid);
  if (user) {
    req.user = user;
  }
};

const getUserData = function (userDetails) {
  const userName = userDetails.username;
  let user = this.todoApp.getUser(userName);
  if (!user) {
    const name = userDetails.name;
    user = this.todoApp.addUser(name, userName);
  }
  return user;
};

const handleUserLogout = function(req, res){
  const sessionid = req.user.sessionid;
  this.sessionManager.logoutUser(sessionid);
  res.setHeader('Set-Cookie', `sessionid='' Max-Age=0`);
  res.redirect('/');
};

const handleUserLogin = function(req, res){
  const sessionid = this.sessionManager.logInUser(req.body.userId);
  if (sessionid) {
    res.setHeader('Set-Cookie', `sessionid=${sessionid}`);
    res.redirect('/todolists');
    return;
  }
  res.setHeader('Set-Cookie', `message=Login Failed; Max-Age=5`);
  res.redirect('/login');
};

module.exports = {
  serveLoginPage,
  redirectLoggedInUserToHome,
  redirectLoggedOutUserToLogin,
  serveTodolists,
  addTodoList,
  setListId,
  serveTodoItems,
  addTodoItem,
  editTodoList,
  deleteTodoList,
  editTodoItem,
  deleteTodoItem,
  loadUser,
  getUserData,
  handleUserLogout,
  handleUserLogin
};
