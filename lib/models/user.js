...
class User {
  ...
  getAllTodoLists() {
    ...
    const allTodoLists = todoListIds.map((listId) => {
      ...
      return { title: todoList.title, id: todoList.listId, description: todoList.description };
    });
    return allTodoLists;
  }
  ...
  updateTodoList(listId, newTitle, newDescription) {
    const todoList = this._todoLists[listId];
    if (todoList) {
      todoList.updateTitleAndDes(newTitle, newDescription);
      return true;
    }
    return false;
  }
